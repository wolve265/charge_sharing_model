...
class PcsModel1DTyped(PcsModel1D):
    """
    The one dimensional model of pixel charge sharing
    with different types enabled
    """

    num_type: type[np.integer] | type[np.floating] = np.float32
    dimension = "unknown"
    gauss_lut: list[num_type] = []
    gauss_bin_size = num_type(0.0)
    ...
    def __init__(
        ...
        PcsModel1DTyped.num_type = num_type
